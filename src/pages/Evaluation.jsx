  ...
  return (
    <div>
      ...
      {activePlayers.length === 0 ? (
        <div className="mb-6 bg-navy-mid border border-white/10 rounded-xl p-4 text-center py-8 text-cream/40">
          <p className="text-sm">Add active players in the Squad tab first.</p>
        </div>
      ) : (
        <div className="mb-6 bg-navy-mid border border-white/10 rounded-xl p-4 flex flex-col md:flex-row md:items-center gap-4">
              ...
              {activePlayers.map(p => {
                return (
                  <option key={p.id} value={p.id}>
                    {p.name} {p.nickname ? `"${p.nickname}"` : ''} ({p.ageGroup})
                  </option>
                )
              })}
            </select>
          ...
          {selected && (
            <div className="flex-1 border-t md:border-t-0 md:border-l border-white/10 pt-4 md:pt-0 md:pl-6 flex flex-col justify-center">
              <div className="flex items-center gap-2 flex-wrap">
                <span className="font-semibold text-cream text-lg leading-tight truncate">{selected.name}</span>
                {selected.nickname && (
                  <span className="text-xs font-semibold px-2 py-0.5 rounded bg-gold/15 text-gold">"{selected.nickname}"</span>
                )}
              </div>
              <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-gold/10 text-gold">{selected.ageGroup}</span>
                <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-white/10 text-cream/70">{selected.position}</span>
                <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-white/5 text-cream/40">
                  {playerEvals.length} eval{playerEvals.length !== 1 ? 's' : ''}
                </span>
                {selected.dob && (
                  <span className="text-xs text-cream/40 font-medium">
                    Age {Math.floor((Date.now() - new Date(selected.dob)) / (365.25 * 24 * 3600 * 1000))}
                  </span>
                )}
                {selected.parentPhone && (
                  <span className="text-xs text-cream/40 flex items-center gap-1 font-medium">
                    📞 {selected.parentPhone}
                  </span>
                )}
              </div>
